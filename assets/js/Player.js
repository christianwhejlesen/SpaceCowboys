...
export default class Player {
   ...
   assetsToLoad = [{ id: 1, var: 'image', src: '../assets/gfx/Player.png' }];
   ...
   draw(ctx) {
      ...
      for (let i = 0; i < this.lives - 1; i++) {
         ctx.drawImage(this.image, i * 30 + 60, 540, this.width, this.height);
      }
   }
      ...
      if (this.assetsToLoad) {
         ...
         for (let i = 0; i < this.assetsToLoad.length; i++) {
            if (this.assetsToLoad[i].var != undefined && this.assetsToLoad[i].src != undefined) {
               this.beginLoadingImage(this.assetsToLoad[i].var, this.assetsToLoad[i].src);
            }
         }
      }
   }
